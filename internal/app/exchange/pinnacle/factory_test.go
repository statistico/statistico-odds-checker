...
func TestMarketFactory_CreateMarket(t *testing.T) {
	t.Run("returns a built market for an event", func(t *testing.T) {
		...
		factory := pinnacle.NewMarketFactory(parser)
		...
		parser.On("ParseMarketOdds", ctx, 55, 70, "OVER_UNDER_25").Return(odds, nil)

		expectedMarket := &exchange.Market{
			ID:       "PIN-55-OVER_UNDER_25",
			Exchange: "PINNACLE",
			Name:     "OVER_UNDER_25",
			EventID:  55,
	...
	t.Run("returns an error if error returned by odds parser", func(t *testing.T) {
		...
		parser.On("ParseMarketOdds", ctx, 55, 70, "OVER_UNDER_25").Return([]sp.Odds{}, errors.New("error from sportmonks"))
	...
	t.Run("returns an error if odds slice returned by odds parser is empty", func(t *testing.T) {
		...
		parser.On("ParseMarketOdds", ctx, 55, 70, "OVER_UNDER_25").Return([]sp.Odds{}, nil)
